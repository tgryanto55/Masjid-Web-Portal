...
const ManageFinance = () => {
    const { state, addTransaction, deleteTransaction, sidebarOpen, showNotification } = useApp();
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-24 sm:pb-12">
            ...
            <div className="space-y-4 sm:space-y-6 pb-24 sm:pb-12 px-4 sm:px-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100 sm:border-0 sm:pb-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Keuangan Masjid</h2>
                            <p className="text-sm text-gray-500">Kelola pemasukan dan pengeluaran operasional masjid.</p>
                        </div>
                    </div>
                </div>

                {/* Summary Cards - Carousel on mobile */}
                <div className="relative">
                    <div
                        ref={scrollRef}
                        onScroll={handleScroll}
                        className="flex md:grid overflow-x-auto md:overflow-x-visible no-scrollbar snap-x snap-mandatory md:grid-cols-3 gap-4 md:gap-6 pb-2 -mx-4 px-4 md:mx-0 md:px-0"
                    >
                        <div className="flex-none w-full md:w-auto snap-center bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-3 sm:space-x-4 transition-all hover:shadow-md">
                            <div className="bg-blue-100 p-2.5 sm:p-4 rounded-lg text-blue-600">
                                <Wallet className="w-5 h-5 sm:w-8 sm:h-8" />
                            </div>
                            <div>
                                <p className="text-gray-500 text-xs sm:text-sm font-medium">Saldo Saat Ini</p>
                                <p className={`text-lg sm:text-2xl font-bold ${balance >= 0 ? 'text-gray-900' : 'text-red-600'}`}>
                        ...
                        <div className="flex-none w-full md:w-auto snap-center bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-3 sm:space-x-4 transition-all hover:shadow-md">
                            <div className="bg-emerald-100 p-2.5 sm:p-4 rounded-lg text-emerald-600">
                                <ArrowUpCircle className="w-5 h-5 sm:w-8 sm:h-8" />
                            </div>
                            <div>
                                <p className="text-gray-500 text-xs sm:text-sm font-medium">Total Pemasukan</p>
                                <p className="text-lg sm:text-2xl font-bold text-emerald-600">{formatRupiah(totalIncome)}</p>
                            </div>
                        </div>

                        <div className="flex-none w-full md:w-auto snap-center bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 flex items-center space-x-3 sm:space-x-4 transition-all hover:shadow-md">
                            <div className="bg-red-100 p-2.5 sm:p-4 rounded-lg text-red-600">
                                <ArrowDownCircle className="w-5 h-5 sm:w-8 sm:h-8" />
                            </div>
                            <div>
                                <p className="text-gray-500 text-xs sm:text-sm font-medium">Total Pengeluaran</p>
                                <p className="text-lg sm:text-2xl font-bold text-red-600">{formatRupiah(totalExpense)}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex sm:hidden justify-center gap-3 mt-4">
                        {[0, 1, 2].map((i) => (
                            <button
                                ...
                                className={`h-2 rounded-full transition-all duration-300 ${activeIndex === i ? 'w-8 bg-emerald-600' : 'w-2 bg-gray-300'
                                    }`}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 grid grid-cols-2 gap-3">
                                        <button
                                            ...
                                            className={`flex items-center justify-center px-4 py-2 border rounded-md text-sm font-medium transition-colors ${formData.type === 'income'
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center justify-center px-4 py-2 border rounded-md text-sm font-medium transition-colors ${formData.type === 'expense'
                                ...
                                <Button
                                    type="submit"
                                    disabled={isSaving}
                                    className="hidden sm:flex w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-900/10 transition-all font-bold items-center justify-center gap-2"
                                >
                                    {isSaving ? (
                                        <>
                            ...
                            <div className="overflow-x-auto flex-1">
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tanggal</th>
                                            <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Keterangan</th>
                                            <th className="px-3 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah</th>
                                            <th className="px-3 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi</th>
                                        </tr>
                                    </thead>
                                            ...
                                            currentTransactions.map((t, index) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 sm:px-6 py-4 text-xs sm:text-sm text-gray-900">
                                                        ...
                                                        {t.category && <div className="text-[10px] sm:text-xs text-gray-400 mt-0.5 inline-block px-1.5 py-0.5 bg-gray-100 rounded">{t.category}</div>}
                                                    </td>
                                                    <td className={`px-3 sm:px-6 py-4 whitespace-nowrap text-xs sm:text-sm text-right font-bold ${t.type === 'income' ? 'text-emerald-600' : 'text-red-600'
                                                        }`}>
                                                        {t.type === 'income' ? '+ ' : '- '}
